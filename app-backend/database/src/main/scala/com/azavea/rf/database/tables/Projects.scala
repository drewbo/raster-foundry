...
object Projects extends TableQuery(tag => new Projects(tag)) with LazyLogging {
      ...
      database.db.run(
        ...
        (p._1, p._2) match {
          case (footprints, Some(project)) => {
            val newScenesExtent = footprints map { footprints => footprints } map { _.get.geom.envelope } reduce { _ combine _ }
            val newProjectExtent = project.extent match {
              case Some(extent) => extent.envelope.combine(newScenesExtent)
              case _ => newScenesExtent
            }

            val newProjectExtentGeometry = new Projected(newProjectExtent.toPolygon(), 3857)
            ...
            database.db.run {
              updateProjectExtentQuery.update(Option(newProjectExtentGeometry))
            }
          }
  ...
  def deleteScenesFromProject(sceneIds: Seq[UUID], projectId: UUID)
                            (implicit database: DB): Future[Int] = {
    val remainingScenesQuery = Scenes
      .filter { scene =>
        scene.id in ScenesToProjects.filter(_.projectId === projectId).filterNot(_.sceneId inSet sceneIds).map(_.sceneId)
      }

    database.db.run {
      remainingScenesQuery.result
    } flatMap { remainingScenes =>
      val newProjectExtentGeometry:Option[Projected[Geometry]] = remainingScenes.length match {
        case 0 => None
        case _ => {
          val extent = remainingScenes map { _.dataFootprint.get.geom.envelope } reduce { _ combine _ }
          Some(new Projected(extent.toPolygon(), 3857))
        }
      }
      ...
      val actions = for {
        project <- updateProjectExtentQuery.update(newProjectExtentGeometry)
